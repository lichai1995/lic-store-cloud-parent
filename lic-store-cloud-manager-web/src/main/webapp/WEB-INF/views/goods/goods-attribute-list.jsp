...
<table class="easyui-datagrid" id="itemParamList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'${ctx }/goods/attribute/getData',method:'get',pageSize:30,toolbar:itemParamListToolbar">
    <thead>
        <tr>
...
<script>
    ...
    var itemParamListToolbar = [{
        ...
        handler:function(){
        	...
        	if(ids.length == 0){
        		$.messager.alert('提示','必须选择一个规格才能编辑!');
        		return ;
        	}
        	if(ids.indexOf(',') > 0){
        		$.messager.alert('提示','只能选择一个规格!');
        		return ;
        	}
        	ego.createWindow({
        		url : "/goods/attribute/edit/"+ids,
        	});
        }
    },{
        text:'删除',
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品规格吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/goods/attribute/delete",params, function(data){
            			if(data.status == 200){
